...
Currently, some joint and marker names are hard coded. Therefore changing these names is not recommended (e.g. 'joint_hip_left', 'joint_hip_right', 'joint_shoulder_left', 'joint_shoulder_left' are hard coded; see functions `get_coord0` and `get_skeleton_coords0` in `anatomy.py`).
